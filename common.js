document.addEventListener("DOMContentLoaded", () => {
	...
	// 시간되면 hover로 준 효과들 js로 변환하자

	// // ▲ section01 hover event **************************************************
	const sticky = document.querySelector(".ab_title");
	const sticky_num = document.querySelector(".ab_title p");
	const viewportMiddle = window.innerHeight / 2
	let section03 = document.querySelector(".section03")
	// console.log(section03.offsetTop)
	...
	window.addEventListener("scroll", () => {
		...
		if (window.innerWidth > 1200) {
			if (rect.top >= viewportMiddle <= window.innerHeight) {
				sticky.style.position = "sticky"
				sticky.style.top = `${viewportMiddle - 50}px`;

				if (window.scrollY >= 3431 && window.scrollY < 4341) {
					sticky_num.textContent = "02"
					sticky.style.opacity = "1"
				} else if (window.scrollY >= 4341 && window.scrollY < 5207) {
					sticky_num.textContent = "03"
					sticky.style.opacity = "1"
				} else if (window.scrollY >= 5207 && window.scrollY < 6150) {
					sticky_num.textContent = "04"
					sticky.style.opacity = "1"
				} else if (window.scrollY >= 6150) {
					sticky.style.opacity = "0"
					sticky.style.transition = ".5s"
				} else {
					sticky_num.textContent = "01"
				}
			}
		} else {
	...
	const img_swiper = new Swiper(".img_mySwiper", {
		centeredSlides: true,
		loop: true,
		simulateTouch: false,
		autoplay: {
